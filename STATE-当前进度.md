...
④左缘残余感知收敛至**底色动态**（R7.5 当前部署）：用户报告两种现象——①色差小时轻微边框/字体抖动 ②色差大时收缩全程可见；元凶=遮盖底色静态采样而任务栏底色随亚克力动态变化（同机实测两时段差 30+RGB）+采样被视频像素污染的三处竞态（采出 #00FFFD/#67EDE8/#C8F2EF 全部实测复现）。R7.5=收缩后延迟 400ms 异步重采样+变化≥3RGB 推送前端即时换色+采样点归属校验三重防御，ffplay 回归通过（进全屏期零采样、收缩后采出本色 #E7D8CD）。**待真机复验④**；主线下一步 Phase 2 输入接管。
...
1. **真机复验④（R7.5）**：PotPlayer 进/出全屏多次（含间隔较久的两轮——让亚克力有机会变色）；核对：①色差场景（情况2）应消失或大幅减轻——每次收缩后 `appearance refresh` 行应采出与当时任务栏一致的色 ②轻微抖动（情况1）应同步减轻（底色匹配后 1px 边缘与文字重栅格化是仅剩痕迹）③日志锚定必须按最近一次 `attached at attempt` 行号（**新陷阱：系统 NTP 回拨致时间戳撞车，昨天 22:25 旧日志会冒充新回归**）。
2. **观察项（未归因，无碍验收）**：a) endorsed 3660↔3649 摆动（安全）；b) y=2071 垂直漂移（自动隐藏任务栏场景）；c) 情况1 的文字重栅格化残留（若仍可感知，候选=WebView 重栅格化，可试 CSS `transform: translateZ(0)` 提层）。
3. **A 基线对照**（评审实验）：`D:\agents_tmp\clock_baseline_probe.ps1 -Tag on/off` 各跑一轮对比回摆频率。
4. 若复验④通过：④项收尾（删 clockrect_debug 标记、删心跳自动化），进主线 **Phase 2 输入接管**（PLAN §3，先 `OVERLAY_TAKEOVER` 开关验证后翻默认）。
